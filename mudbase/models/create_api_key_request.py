...
class CreateApiKeyRequest(BaseModel):
    ...
    project_id: Annotated[str, Field(strict=True)] = Field(description="MongoDB ObjectId of the project", alias="projectId")
    permissions: Optional[List[ApiKeyPermission]] = Field(default=None, description="Optional. Permission objects (resource + actions). Omit or pass [] for full access (all resources and actions). Include only the entries you want; remove resources or actions to restrict the key.")
    rate_limit: Optional[RateLimit] = Field(default=None, alias="rateLimit")
    expires_at: Optional[datetime] = Field(default=None, description="Optional. When provided, must be a valid ISO 8601 date-time in the future. Omit for no expiration.", alias="expiresAt")
    __properties: ClassVar[List[str]] = ["name", "projectId", "permissions", "rateLimit", "expiresAt"]
