...
class AdminOrgPlanPatchRequest(BaseModel):
    """
    ...
    __properties: ClassVar[List[str]] = ["plan", "reason", "txPlan"]

    @field_validator('plan')
    def plan_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['free', 'basic', 'starter', 'growth', 'scale', 'enterprise']):
            raise ValueError("must be one of enum values ('free', 'basic', 'starter', 'growth', 'scale', 'enterprise')")
        return value

    @field_validator('tx_plan')
    def tx_plan_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value
