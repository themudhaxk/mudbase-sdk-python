...
class RestoreBackupRequest(BaseModel):
    ...
    def restore_mode_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['replace', 'merge']):
            raise ValueError("must be one of enum values ('replace', 'merge')")
        return value

    @field_validator('confirmation')
    def confirmation_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['RESTORE_DATA']):
            raise ValueError("must be one of enum values ('RESTORE_DATA')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "restoreMode": obj.get("restoreMode"),
