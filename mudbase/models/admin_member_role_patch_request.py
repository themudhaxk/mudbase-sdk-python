...
class AdminMemberRolePatchRequest(BaseModel):
    """
    AdminMemberRolePatchRequest
    """ # noqa: E501
    role: StrictStr
    reason: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["role", "reason"]

    @field_validator('role')
    def role_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['owner', 'admin', 'developer', 'viewer']):
            raise ValueError("must be one of enum values ('owner', 'admin', 'developer', 'viewer')")
        return value
