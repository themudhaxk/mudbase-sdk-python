...
class TriggerWebhookRequest(BaseModel):
    ...
    """ # noqa: E501
    project_id: StrictStr = Field(description="Target project (must belong to your org)", alias="projectId")
    url: StrictStr = Field(description="HTTPS URL validated against SSRF rules")
    event: StrictStr = Field(description="Event name (sent as X-MUDBASE-Event)")
    payload: Dict[str, Any] = Field(description="JSON body POSTed to your endpoint")
    method: Optional[StrictStr] = 'POST'
    __properties: ClassVar[List[str]] = ["projectId", "url", "event", "payload", "method"]
