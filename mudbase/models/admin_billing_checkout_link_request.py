...
class AdminBillingCheckoutLinkRequest(BaseModel):
    """
    AdminBillingCheckoutLinkRequest
    """ # noqa: E501
    plan: StrictStr
    billing_cycle: Optional[StrictStr] = Field(default='monthly', alias="billingCycle")
    amount_cents: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="Monthly amount in cents (overrides catalog; enterprise default is contract)", alias="amountCents")
    charge_amount_cents: Optional[Annotated[int, Field(strict=True, ge=1)]] = Field(default=None, description="Exact charge in cents for this checkout (overrides monthly math)", alias="chargeAmountCents")
    currency: Optional[StrictStr] = None
    email: Optional[StrictStr] = None
    name: Optional[StrictStr] = None
    redirect_url: Optional[StrictStr] = Field(default=None, alias="redirectUrl")
    send_email: Optional[StrictBool] = Field(default=False, alias="sendEmail")
    to_email: Optional[StrictStr] = Field(default=None, alias="toEmail")
    message: Optional[StrictStr] = Field(default=None, description="Optional note shown in org_billing_checkout email")
    __properties: ClassVar[List[str]] = ["plan", "billingCycle", "amountCents", "chargeAmountCents", "currency", "email", "name", "redirectUrl", "sendEmail", "toEmail", "message"]

    @field_validator('plan')
    def plan_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['starter', 'growth', 'scale', 'enterprise']):
            raise ValueError("must be one of enum values ('starter', 'growth', 'scale', 'enterprise')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "plan": obj.get("plan"),
            "billingCycle": obj.get("billingCycle") if obj.get("billingCycle") is not None else 'monthly',
            "amountCents": obj.get("amountCents"),
            "chargeAmountCents": obj.get("chargeAmountCents"),
            "currency": obj.get("currency"),
            "email": obj.get("email"),
            "name": obj.get("name"),
            "redirectUrl": obj.get("redirectUrl"),
            "sendEmail": obj.get("sendEmail") if obj.get("sendEmail") is not None else False,
