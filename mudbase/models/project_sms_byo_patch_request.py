...
class ProjectSmsByoPatchRequest(BaseModel):
    """
    ProjectSmsByoPatchRequest
    """ # noqa: E501
    enabled: Optional[StrictBool] = None
    provider: Optional[StrictStr] = None
    default_from: Optional[StrictStr] = Field(default=None, description="Default sender (E.164 for Twilio; Termii/Africa's Talking may use alphanumeric or approved sender IDs per provider rules).", alias="defaultFrom")
    config: Optional[Dict[str, Any]] = Field(default=None, description="Provider credentials and options (encrypted at rest). Required keys when enabling BYO: **twilio** — `accountSid`, `authToken`. Optional `from`. **termii** — `apiKey`. Optional `from`. **africastalking** — `username`, `apiKey`. Optional `from`. ")
    __properties: ClassVar[List[str]] = ["enabled", "provider", "defaultFrom", "config"]
    ...
    def provider_validate_enum(cls, value):
        ...
        if value not in set(['twilio', 'termii', 'africastalking', 'none']):
            raise ValueError("must be one of enum values ('twilio', 'termii', 'africastalking', 'none')")
        return value
