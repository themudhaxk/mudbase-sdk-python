...
class InitializeOrgPlanCheckoutRequest(BaseModel):
    """
    InitializeOrgPlanCheckoutRequest
    """ # noqa: E501
    plan_name: StrictStr = Field(description="Plan id from GET /api/billing/plans (excludes free and enterprise)", alias="planName")
    billing_cycle: Optional[StrictStr] = Field(default='monthly', description="Yearly = 2 months free (~16.67% discount)", alias="billingCycle")
    redirect_url: Optional[StrictStr] = Field(default=None, description="Override redirect after payment (default FRONTEND_URL/billing/callback)", alias="redirectUrl")
    __properties: ClassVar[List[str]] = ["planName", "billingCycle", "redirectUrl"]

    @field_validator('plan_name')
    def plan_name_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['starter', 'growth', 'scale']):
            raise ValueError("must be one of enum values ('starter', 'growth', 'scale')")
        return value
