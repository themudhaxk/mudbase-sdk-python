...
class ApiKeyPermission(BaseModel):
    ...
    ApiKeyPermission
    """ # noqa: E501
    resource: StrictStr = Field(description="Resource scope for this permission (auth, database, storage, functions, realtime, messaging)")
    actions: List[StrictStr] = Field(description="Allowed actions on the resource")
    __properties: ClassVar[List[str]] = ["resource", "actions"]

    @field_validator('resource')
    def resource_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['auth', 'database', 'storage', 'functions', 'realtime', 'messaging']):
            raise ValueError("must be one of enum values ('auth', 'database', 'storage', 'functions', 'realtime', 'messaging')")
        return value
