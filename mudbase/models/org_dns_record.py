...
class OrgDnsRecord(BaseModel):
    """
    OrgDnsRecord
    """ # noqa: E501
    type: StrictStr = Field(description="DNS record type (TXT, CNAME, …)")
    name: StrictStr = Field(description="Owner name / FQDN to create at the customer's DNS host")
    value: StrictStr = Field(description="Record value or CNAME target")
    purpose: StrictStr = Field(description="mudbase_ownership, routing, fly_ownership, acme_challenge, or fly (legacy bucket).")
    __properties: ClassVar[List[str]] = ["type", "name", "value", "purpose"]
