...
class SendMessageRequest(BaseModel):
    ...
    """ # noqa: E501
    type: StrictStr
    content: Optional[StrictStr] = Field(default=None, description="Plaintext body; omit when sending e2ee (use e2ee.ciphertext for E2EE text)")
    e2ee: Optional[SendMessageRequestE2ee] = None
    reply_to: Optional[StrictStr] = Field(default=None, alias="replyTo")
    mentions: Optional[List[StrictStr]] = None
    __properties: ClassVar[List[str]] = ["type", "content", "e2ee", "replyTo", "mentions"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['text', 'image', 'video', 'audio', 'file', 'location', 'contact']):
            raise ValueError("must be one of enum values ('text', 'image', 'video', 'audio', 'file', 'location', 'contact')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of e2ee
        if self.e2ee:
            _dict['e2ee'] = self.e2ee.to_dict()
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "e2ee": SendMessageRequestE2ee.from_dict(obj["e2ee"]) if obj.get("e2ee") is not None else None,
