...
class FunctionTrigger(BaseModel):
    """
    FunctionTrigger
    """ # noqa: E501
    type: StrictStr = Field(description="Trigger type")
    event: Optional[StrictStr] = Field(default=None, description="Event name (e.g. create, update, delete for document; uploaded, deleted for file)")
    schedule: Optional[StrictStr] = Field(default=None, description="For cron - minutely, hourly, daily, weekly, or custom cron expression")
    path: Optional[StrictStr] = Field(default=None, description="HTTP path for http triggers")
    method: Optional[StrictStr] = None
    collection_id: Optional[StrictStr] = Field(default=None, description="For document triggers - filter by collection", alias="collectionId")
    bucket_id: Optional[StrictStr] = Field(default=None, description="For file triggers - filter by bucket", alias="bucketId")
    __properties: ClassVar[List[str]] = ["type", "event", "schedule", "path", "method", "collectionId", "bucketId"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['http', 'event', 'document', 'file', 'webhook', 'cron', 'messaging']):
            raise ValueError("must be one of enum values ('http', 'event', 'document', 'file', 'webhook', 'cron', 'messaging')")
        return value
